...
static int callback_dump(struct nl_msg *msg, void *arg) {
    ...
    // Radiotap + 802.11 frame + beacon headers
    static char * packet_header = "\x00\x00\x0d\x00\x28\x00\x00\x00\xff\xff\x00\x01\xff\x80\x00" \
    "\x00\x00\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff" \
    "\xff\xff\xff\x00\x00\xff\xff\xff\xff\xff\xff\x00\x00\x00\x00\x01\x00";
    int packet_header_len = 49;
...
int do_scan_trigger(struct nl_sock *socket, int if_index, int genl_id) {
    ...
    int err;
    int ret;
    int mcid = genl_ctrl_resolve_grp(socket, NL80211_GENL_FAMILY_NAME, NL80211_GENL_GROUP_NAME);
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        nla_put_u32(msg, NL80211_ATTR_IFINDEX, if_index);
        nl_socket_modify_cb(socket, NL_CB_VALID, NL_CB_CUSTOM, callback_dump, dumper);
